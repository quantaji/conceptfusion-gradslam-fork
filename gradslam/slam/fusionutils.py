...
def are_normals_similar(
    ...
    if dot_res.max() > 1.001:
        warnings.warn(
            "Max of dot product was {0} > 1. Inputs were not normalized along dim ({1}). Was this intentional?".format(dot_res.max(), dim),
            RuntimeWarning,
        )
...
def find_active_map_points(
    ...
    pointclouds_transformed = pointclouds.partial_clone_transform(transform=tinv, clone_points=True)
    is_front_of_plane = (pointclouds_transformed.points_padded[..., -1] > 0)  # don't consider missing depth values
    pointclouds_transformed.pinhole_projection_(rgbdimages.intrinsics.squeeze(1))  # IN-PLACE operation
    img_plane_points = pointclouds_transformed.points_padded[..., :-1]  # width, height

    is_in_frame = ((img_plane_points[..., 0] > -1e-3) & (img_plane_points[..., 0] < width - 0.999) & (img_plane_points[..., 1] > -1e-3) & (img_plane_points[..., 1] < height - 0.999) & is_front_of_plane & pointclouds.nonpad_mask)
    ...
    batch_point_idx = (create_meshgrid(batch_size, num_points, normalized_coords=False).squeeze(0).to(device))
...
def find_similar_map_points(
    ...
    if len(pc2im_bnhw_similar) == 0:
        warnings.warn(
            "No similar map points were found (despite total {0} active points across the batch)".format(pc2im_bnhw.shape[0]),
            RuntimeWarning,
        )
...
def find_best_unique_correspondences(
    ...
    inv_ccounts = 1 / (pointclouds.features_padded[pc2im_bnhw[:, 0], pc2im_bnhw[:, 1]] + 1e-20)  # shape: [P 1]
    # compute ray dist
    ...
    sorted_criteria = torch.unique(unique_criteria_bhwcrn.detach(), dim=0)  # pytorch issue #47851
...
def fuse_with_map(
    ...
    if pointclouds.has_points:
        ...
        if use_embeddings:
            if not pointclouds.has_embeddings:
                ...
                raise ValueError("Pointclouds must have embeddings for map fusion, but did not.")
    ...
    if pointclouds.has_points and pc2im_bnhw.shape[0] != 0:
        ...
        map_ccounts = pointclouds.features_padded[pc2im_bnhw[:, 0], pc2im_bnhw[:, 1]]
        alpha = alpha_image[pc2im_bnhw[:, 0], 0, pc2im_bnhw[:, 2], pc2im_bnhw[:, 3]]
        updated_ccounts = map_ccounts + alpha

        # points
        pointclouds.points_padded[pc2im_bnhw[:, 0], pc2im_bnhw[:, 1]] *= map_ccounts / updated_ccounts
        pointclouds.points_padded[pc2im_bnhw[:, 0], pc2im_bnhw[:, 1]] += alpha * vertex_maps[pc2im_bnhw[:, 0], 0, pc2im_bnhw[:, 2], pc2im_bnhw[:, 3]] / updated_ccounts

        # normals
        pointclouds.normals_padded[pc2im_bnhw[:, 0], pc2im_bnhw[:, 1]] *= map_ccounts / updated_ccounts
        pointclouds.normals_padded[pc2im_bnhw[:, 0], pc2im_bnhw[:, 1]] += alpha * normal_maps[pc2im_bnhw[:, 0], 0, pc2im_bnhw[:, 2], pc2im_bnhw[:, 3]] / updated_ccounts

        # colors
        pointclouds.colors_padded[pc2im_bnhw[:, 0], pc2im_bnhw[:, 1]] *= map_ccounts / updated_ccounts
        pointclouds.colors_padded[pc2im_bnhw[:, 0], pc2im_bnhw[:, 1]] += alpha * rgb_image[pc2im_bnhw[:, 0], 0, pc2im_bnhw[:, 2], pc2im_bnhw[:, 3]] / updated_ccounts

        # cconts
        pointclouds.features_padded[pc2im_bnhw[:, 0], pc2im_bnhw[:, 1]] = updated_ccounts

        # If with embeddings, fuse them accordingly
        if pointclouds.has_embeddings:  # KM

            frame_embeddings = embeddings[pc2im_bnhw[:, 0], 0, pc2im_bnhw[:, 2], pc2im_bnhw[:, 3]]
            ...
            valid_embeddings_mask = (torch.count_nonzero(frame_embeddings, dim=-1) > 0).unsqueeze(-1)

            # If RGBDImages has confidences, use it as the fusion factor
            if confidence_image is not None:  # KM
                frame_confidences = confidence_image[pc2im_bnhw[:, 0], 0, pc2im_bnhw[:, 2], pc2im_bnhw[:, 3]]

            # If not, use the alpha as the fusion factor
            else:
                frame_confidences = alpha
            ...
            if embedding_fusion_method == "slam":
                ...
                frame_embeddings[frame_confidences.squeeze(-1) <= 0] = 1.0
                pointclouds.embeddings_padded[pc2im_bnhw[:, 0], pc2im_bnhw[:, 1]] *= frame_embeddings

            else:
                raise ValueError(f'Unknown embedding_fusion_method "{embedding_fusion_method}"')
    ...
    new_mask = new_mask * rgbdimages.valid_depth_mask.squeeze(-1)  # don't add missing depths to map
